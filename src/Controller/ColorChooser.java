...
public class ColorChooser extends JPanel implements ActionListener, ChangeListener {
	...
	private JSlider viewScaleSlider;
	private JLabel viewScaleValueLabel;
	...
	public ColorChooser(DrawFrame frame) {
		...
		viewScaleSlider.addChangeListener(this);
	}
	...
	private void initializeColorChooser(JToolBar panel) {
		...
		viewScaleSlider = new JSlider(JSlider.HORIZONTAL, 10, 400, 100);
		viewScaleSlider.setPreferredSize(new Dimension(200, 24));
		viewScaleSlider.setMajorTickSpacing(50);
		viewScaleSlider.setMinorTickSpacing(10);
		viewScaleSlider.setPaintTicks(false);
		viewScaleValueLabel = new JLabel("100%");
		...
		panel.add(option);
		panel.add(fill);
		panel.add(new JLabel("View Scale"));
		panel.add(viewScaleSlider);
		panel.add(viewScaleValueLabel);
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == fill) {
			...
			if (b == custom2) {
				...
				Color initialColor = (option.getSelectedItem() == PRIMARY_COLOR)
						? primaryColor.getBackground()
						: secondaryColor.getBackground();
				Color chosenColor = JColorChooser.showDialog(frame, "Choose Color", initialColor);

				if (chosenColor != null) {
					if (option.getSelectedItem() == PRIMARY_COLOR) {
						custom3.setBackground(custom2.getBackground());
						custom2.setBackground(primaryColor.getBackground());
						primaryColor.setBackground(chosenColor);
					} else {
						custom3.setBackground(custom2.getBackground());
						custom2.setBackground(secondaryColor.getBackground());
						secondaryColor.setBackground(chosenColor);
					}
				}
			}
	...
	public void stateChanged(ChangeEvent e) {
		if (e.getSource() == viewScaleSlider) {
			int percent = viewScaleSlider.getValue();
			viewScaleValueLabel.setText(percent + "%");
			frame.getInkPanel().setViewScale(percent / 100.0);
		}
	}
